...
/**
 * 会场组件管理器（可复用）。
 * - 独立页 /admin/venue-components 里传 showHeader 渲染完整 sticky 顶栏
 * - /admin 里的 tab 模式不传 showHeader，改为顶部右对齐一个小 action bar
 */
export function VenueComponentsManager({
  showHeader = false,
}: {
  showHeader?: boolean;
}) {
  ...
  const visibleGroups = VENUE_COMPONENT_GROUPS.filter(
    (g) => (byGroup.get(g) ?? []).length > 0,
  );
  const shownGroups = venueFilterGroup
    ? [venueFilterGroup]
          ...
          <div className="space-y-8">
            {components.length > 0 && (
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() => setVenueFilterGroup(null)}
                  className={[
                    "rounded-full border px-3 py-1 text-xs transition-colors",
                    venueFilterGroup === null
                ...
                {visibleGroups.map((g) => {
                  const count = (byGroup.get(g) ?? []).length;
                  const active = venueFilterGroup === g;
                  return (
                    <button
                      key={g}
                      type="button"
                      onClick={() => setVenueFilterGroup(g)}
                      className={[
                        "rounded-full border px-3 py-1 text-xs transition-colors",
                        active
                          ? "border-gray-900 bg-gray-900 text-white"
